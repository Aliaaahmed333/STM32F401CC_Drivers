...
/* Parameters in  :  void                                              *
/* Parameters out :  void                                              *
/* Description    :  select RCC clock for system HSE,HSI OR PLL        *
/***********************************************************************/
...
/**************************************************************************
* Api Name       :  RCC_InitForRTC_Clk                                    *
* Parameters in  :  void                                                  *
* Parameters out :  void                                                  *
* Description    :  this function enable clock for rtc using LSI and then *
*                :  enable RTCEN pin in backup register                   *
***************************************************************************/
void  RCC_InitForRTC_Clk (void)
{
	/* Enable LSI clk for RTC and wait for ready flag */
	  SET_BIT(RCC_CSR ,LSION);
	  while (GET_BIT(RCC_CSR,LSIRDY) == 0);

	/*RTC CLOCK SOURCE SELECTION  WE use LSI 10: LSI oscillator clock used as the RTC clock*/
	  CLR_BIT(RCC_BDCR,RTCSEL0);
	  SET_BIT(RCC_BDCR,RTCSEL1);
